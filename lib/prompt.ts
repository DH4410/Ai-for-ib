...
const modeRules: Record<StudyMode, string> = {
  ...
  mark:
    "Act as a careful IB-style marker. Identify correct working, missing reasoning, unit/significant-figure errors and what is needed for the next mark. If an official markscheme is present in retrieved context, use it explicitly. For a retrieved real past-paper question, award a numeric mark and emit a MARK footer only when the official markscheme is present. If that real question is question-only, give qualitative feedback and say that an official numeric score is unavailable; do not invent a MARK footer. For non-past-paper work, only use a numeric MARK footer when a trustworthy maximum mark is supplied. When a numeric footer is allowed, finish with exactly one final line in the form MARK: awarded/maximum using numeric values and write nothing after it.",
  revise:
